...
- **python3** — used internally to build correct `file://` bookmark URIs (handles spaces/unicode in paths safely).
...
1. Installs dependencies (`archivemount`, `gvfs`, `python3`, `nemo`) using whichever package manager it finds (`apt-get`, `dnf`, `pacman`, or `zypper`).
